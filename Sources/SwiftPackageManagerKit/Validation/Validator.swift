...
public struct Validator: Sendable {
  ...
  public func validatePackageStructure(_ package: PackageInfo) -> [ValidationIssue] {
    ...
    for target in package.targets where target.dependencies.isEmpty && target.type == .regular {
      ...
      for dependency in target.dependencies {
        ...
        case .product(let productName, let packageName, _):
          ...
          if !dependencyNames.contains(packageName) {
            issues.append(
              .missingDependency(
                target: target.name, dependency: "\(productName) from \(packageName)"))
          }
        }
      }
    }
            ...
            if compareSemanticVersions(lowerBound, upperBound) >= 0 {
              issues.append(
                .invalidVersionRequirement(
                  dependency: identity,
                  requirement:
                    "Lower bound \(lowerBound) is not less than upper bound \(upperBound)"
                ))
            }
          }
  ...
  public func detectCircularDependencies(in package: PackageInfo) -> [ValidationIssue] {
    ...
    for target in package.targets {
      ...
      if hasCycle(
        target: target.name, targetMap: targetMap, visited: &visited,
  ...
  public func findOrphanedTargets(in package: PackageInfo) -> [ValidationIssue] {
    ...
    for target in package.targets where !usedTargets.contains(target.name) && target.type != .test {
    ...
    if let targetInfo = targetMap[target] {
      for dependency in targetInfo.dependencies {
        ...
        case .byName(let depName, _):
          if hasCycle(
            target: depName, targetMap: targetMap, visited: &visited,
            recursionStack: &recursionStack, path: &path)
          {
            return true
          }
